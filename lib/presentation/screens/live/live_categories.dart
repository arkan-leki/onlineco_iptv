    ...
    return Scaffold(
      floatingActionButton: Visibility(
        ...
        child: FloatingActionButton(
          onPressed: () {
            setState(() {
              _hideButtonController.animateTo(0, duration: const Duration(milliseconds: 400), curve: Curves.ease);
              _hideButton = true;
            });
          },
      ...
      body: Stack(
        ...
        children: [
          Ink(
            ...
            child: Column(
              children: [
                // App bar similar to KurdishLiveChannelsScreen
                AppBarLiveCategory(
                  // onChannels: () {
                      ...
                      return [
                        SliverAppBar(
                          ...
                          flexibleSpace: FlexibleSpaceBar(
                            background: const SizedBox(), // Empty because we already added the app bar above
                          ),
                        ),
                      ];
                    },
                      ...
                      builder: (context, state) {
                        if (state is LiveCatyLoading) {
                          ...
                          List<CategoryModel> searchCaty = categories
                              .where((element) => element.categoryName!.toLowerCase().contains(keySearch))
                              .toList();

                          return GridView.builder(
                            padding: const EdgeInsets.only(left: 10, right: 10, top: 0, bottom: 80),
                            itemCount: keySearch.isNotEmpty ? searchCaty.length : categories.length,
                            ...
                            itemBuilder: (_, i) {
                              final model = keySearch.isNotEmpty ? searchCaty[i] : categories[i];

                              return CardLiveItem(
                                title: model.categoryName ?? "",
                                onTap: () {
                                  Get.to(() => LiveChannelsScreen(catyId: model.categoryId ?? ''))!
                                      .then((value) async {
                                    // Optional ad or reload actions can go here
                                  });
                                },
                              );
                            },
                        ...
                        return const Center(child: Text("Failed to load data... blew blew"));
                      },
                    ),
                  ),
                ),
